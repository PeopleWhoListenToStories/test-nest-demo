...
export class FileService {
  ...
  async uploadFile(file, unique): Promise<File> {
    ...
    console.log(`%c 🏄‍♂️ 🚀 : FileService -> originalname, mimetype, size, buffer `, `font-size:14px;background-color:#f27590;color:white;`, originalname, mimetype, size);
    const filename = +unique === 1 ? `/${dateFormat(new Date(), 'yyyy-MM-dd')}/${uniqueid()}/${originalname}` : `/${dateFormat(new Date(), 'yyyy-MM-dd')}/${originalname}`
    console.log(`%c 🔠 🚀 : FileService -> filename `, `font-size:14px;background-color:#c4d976;color:black;`, filename);
