...
import { IUser, IDocument, IOrganization, RedisDBEnum } from '~/constant'
import { Document as DocumentEntity } from '~/modules/document/document.entity'
...
export class ViewService {
  ...
  private async buildRedis() {
    try {
      ...
      console.log('[slayKit-server] 访问记录服务启动成功')
    } catch (e) {
      console.error(`[slayKit-server] 访问记录服务启动错误: "${e.message}"`)
    }
  }
   ...
   * @param documentId
   */
  public async deleteDeletedDocumentView(user: IUser, organizationId: IOrganization['id'], documentId: IDocument['id']) {
    const { id: userId } = user
    const key = this.buildVisitedDocumentInOrganizationKey(organizationId)
    const oldData = await this.redis.hget(key, userId)
  ...
  async create(user: IUser | null, document: DocumentEntity) {
    await Promise.all([this.recordDocumentViews(document.id), user && this.recordUserVisitedDocumentInOrganization(user, document)])
  }
